...
describe('NoteRepository Interface', () => {
  ...
  it('should throw error when invoke abstract behavior', () => {
    ...
    const noteRepository = new NoteRepository();
    // Action adn Assert
    expect(noteRepository.addNote({})).rejects.toThrowError('NOTE_REPOSITORY.METHOD_NOT_IMPLEMENTED');
    expect(noteRepository.editNote({})).rejects.toThrowError('NOTE_REPOSITORY.METHOD_NOT_IMPLEMENTED');
    expect(noteRepository.deleteNote('')).rejects.toThrowError('NOTE_REPOSITORY.METHOD_NOT_IMPLEMENTED');
  });
});
